...
class lmbAclConfiguratorTest extends UnitTestCase
{
  ...
  function testAllowRuleWithMask()
  {
	...
	$configurator = new lmbAclConfigurator(array_merge($options, array('allow' => array('guest'))));
    ...
    $configurator = new lmbAclConfigurator(array_merge($options, array('allow' => array('guest::read'))));
  ...
  function testEnableDefaultAllowPolicy()
  {
    $options = array(
	  'roles' => array('guest' => null),
	  'resources' => array('article' => null),
	  'allow' => array('guest:article')
	  );
	  
    $configurator = new lmbAclConfigurator($options);
	$this->assertFalse($configurator->getAcl()->isAllowed('guest', 'article', 'read'));
