...
class lmbMacroFormCheckboxTagTest extends lmbBaseMacroTest
{
  function testRenderHiddenWithCheckbox()
  {
    $template = '{{form:checkbox name="my_checkbox" value="$#var"/}}';
  ...
  {
    $template = '{{form:checkbox name="my_checkbox" value="$#var" checked="checked"/}}';
  ...
  function testRenderCheckboxInStandartMode()
  {
    $template = '{{form:checkbox name="my_checkbox" value="$#var" standart_mode="1"/}}';
    ...
    $this->assertEqual(1, count($html->input));
    $this->assertEqual($html->input[0]['type'], 'checkbox');
  }
}
