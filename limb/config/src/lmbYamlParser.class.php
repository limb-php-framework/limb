...
require_once (dirname(__FILE__).'/lmbYamlInline.class.php');
...
/**
 * lmbYamlParser parses YAML strings to convert them to PHP arrays.
 *
 * @package    symfony
 * @subpackage yaml
 * @author     Fabien Potencier <fabien.potencier@symfony-project.com>
 * @version    SVN: $Id: lmbYamlParser.class.php 10832 2008-08-13 07:46:08Z fabien $
 */
class lmbYamlParser
{
  protected
    ...
    while ($this->moveToNextLine())
    {
      ...
      if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        ...
        else
        {
          ...
          {
            $data[] = array($matches[1] => lmbYamlInline::load($matches[2]));
          }
          elseif (isset($values['leadspaces'])
            && ' ' == $values['leadspaces']
            && preg_match('#^(?P<key>'.lmbYamlInline::REGEX_QUOTED_STRING.'|[^ \'"\{].*?) *\:(\s+(?P<value>.+?))?\s*$#', $values['value'], $matches))
          {
      ...
      else if (preg_match('#^(?P<key>'.lmbYamlInline::REGEX_QUOTED_STRING.'|[^ \'"].*?) *\:(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values))
      {
        $key = lmbYamlInline::parseScalar($values['key']);
      ...
      else
      {
        ...
        {
          $value = lmbYamlInline::load($this->lines[0]);
          if (is_array($value))
          {
            $first = reset($value);
            if ('*' === substr($first, 0, 1))
  ...
  protected function parseValue($value)
  {
    ...
    {
      return lmbYamlInline::load($value);
    }
  }
