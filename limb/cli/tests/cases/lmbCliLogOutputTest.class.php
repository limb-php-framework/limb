...
lmb_require('limb/log/src/lmbLogMemoryWriter.class.php');
...
class lmbCliLogOutputTest extends UnitTestCase
{
  ...
  {
    $log = new lmbLog;
    $log->registerWriter(new lmbLogMemoryWriter(new lmbUri()));

    $out = new lmbCliLogOutput($log);
    $out->write('message', array('param1' => 'value1'));

    $entry = current(current($log->getWriters())->getEntries());
