...
var tb_pathToImage = "/shared/cms/images/icons/loading.gif";
...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
  ...
  try {
    ...
    if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
      ...
      $("#TB_closeWindowButton").click(tb_remove);
      $("#TB_resizeWindowButton").click(tb_resizeWindow);
...
function tb_resizeWindow()
{
  var resizeBtn = jQuery('#TB_resizeWindowButton');
  ...
  {
    tb_window.css({left: '16px', top:'16px', width: (page_size[0]-40) + 'px', height: (page_size[1]-40) + 'px', margin:'0px'});
    tb_iframe.css({width: (page_size[0]-40) + 'px', height: (page_size[1]-67) + 'px'});
    resizeBtn.removeClass('maximize_window').addClass('minimize_window').attr({title: title_min, alt: title_min});
  }
  else
  {
    tb_position();
    tb_iframe.css({width: TB_WIDTH + 'px', height: TB_HEIGHT + 'px'});
    resizeBtn.removeClass('minimize_window').addClass('maximize_window').attr({title: title_max, alt: title_max});
  }
}
