 ...
 * @version    $Id: WactDateFilterTest.class.php 5021 2007-02-12 13:04:07Z pachanga $
 * @package    wact
 */

class WactDateFilterTest extends WactTemplateTestCase
{
    function testSimpleUse()
    {
      $template = '{$test|date:"Ymd"}';
      $this->registerTestingTemplate('/filters/core/date/var.html', $template);

      $page = $this->initTemplate('/filters/core/date/var.html');
      $page->set('test', mktime(0,0,0,date('m'),date('d'),date('y')));
      $output = $page->capture();
      $this->assertEqual($output, strftime('%Y%m%d'));
    }

}

class WactToDateFilterTestCase extends WactTemplateTestCase {
