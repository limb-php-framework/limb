...
class WactPerformComponent extends WactRuntimeComponent
{
  ...
  function process($template)
  {
    ...
    if(!is_a($command, 'WactTemplateCommand'))
      throw new lmbException($this->command. '" must inherite from WactTemplateCommand class');

    $method = WactTemplate :: toStudlyCaps('do_' . $this->method_name, false);
    if(!method_exists($command, $method))
      throw new lmbException('Template command "' .$this->command. '" does not support method: '. $method);
