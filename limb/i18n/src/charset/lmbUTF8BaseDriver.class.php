...
class lmbUTF8BaseDriver {
    ...
    */
    function _substr($str, $start, $length=null) {
        ...
        if (!is_null($length) && abs($length) > $strlen)
            $length = ($length > 0) ? $strlen : -1 * $strlen;
        ...
        if (is_null($length) || $length >= $strlen)
            $length = '*';
        else
            $length = '{0,' . $length . '}';
    ...
    */
    function _trim($str, $charlist = '') {
        ...
        return $this->_ltrim($this->_rtrim($str, $charlist), $charlist);
    }
    ...
    */
    function _strpos($haystack, $needle, $offset=false) {
        if ($offset === false) {
            ...
            $haystack = $this->_substr($haystack, $offset);

            if (false !== ($pos = $this->_strpos($haystack, $needle)))
                return $pos + $offset;
